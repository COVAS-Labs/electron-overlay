...
if (!ref.ok) throw new Error(`Unable to read remote tag ${tag}: ${ref.status} ${await ref.text()}`);
let object = (await ref.json()).object;
for (let depth = 0; object.type === "tag" && depth < 5; depth += 1) {
  const annotatedTag = await githubRequest(`/repos/${repository}/git/tags/${object.sha}`);
  if (!annotatedTag.ok) {
    throw new Error(`Unable to dereference annotated tag ${object.sha}: ${annotatedTag.status} ${await annotatedTag.text()}`);
  }
...
if (release.status === 404) {
  ...
  if (!release.ok) throw new Error(`Unable to query GitHub Release ${tag}: ${release.status} ${await release.text()}`);
